package spaceraze.client.game.map;
...
public class MapCanvas extends SRBasePanel implements Runnable, MouseListener, MouseMotionListener {
	...
	double zvalue = 100.0, widthoffset, heightoffset, zoffset, animation = Math.PI / 2, tempWidthOffset = 0,
			tempHeightOffset = 0, tempzoffset = 0;
	Player player;
	Thread t;
	ShowMapPlanet aShowMapPlanet;
	String chosenCoors = "", newCenterString = "";
	Coors origo;
	boolean once = true, doChange = false, doReset = false, startChange = false, mouseDragged = false,
			stopChange = false, showVips = false;
	...
	public MapCanvas(Player player, ShowMapPlanet aShowMapPlanet, boolean drawEdge) {
		 ...
		 * controls.setBounds(1,65,120,370); add(controls);
		 */
	...
	public void newPcoors() {
		...
		c.setMapPlanetUuid("");
		c.setOrigo(true);
		c.setColor(new Color(0, 127, 0)); // m�rkgr�n
	...
	public void computeNewOrigo() {
		...
		for (int i = 0; i < pcoors.size(); i++) {
			...
			} else if (!tempCoor.isConnection()) { // is a planet
	...
	public void run() {
		for (;;) {
			...
			// System.out.println("Values: " + xoffset + yoffset + roffset + oldX + oldY +
			// widthoffset + heightoffset + " z: " + zoffset);
			// paint(getGraphics());
	...
	public void mousePressed(MouseEvent me) {
		...
		if (foundCoors != null) {
			animation = Math.PI / 2;
			chosenCoors = foundCoors.getMapPlanetUuid();
			if (me.getModifiers() == 16) { // if left mousebutton pressed
				// System.out.println("showPlanet: " + me.getModifiers());
				aShowMapPlanet.showPlanet(foundCoors.getMapPlanetUuid());
			} else if (me.getModifiers() == 4) { // if right mousebutton pressed
				// System.out.println("showPlanetShips: " + me.getModifiers());
				if (!player.isDefeated() & !SpaceRazePanel.galaxy.isGameOver()) {
					aShowMapPlanet.showPlanetShips(foundCoors.getMapPlanetUuid());
				}
			}
			// paint(getGraphics());
		}
	...
	public void mouseDragged(MouseEvent me) {
		...
		/*
		 * if (t == null){ t = new Thread(this); t.start(); }else{ t.stop(); t = new
		 * Thread(this); t.start(); }
		 */
		...
		if (me.getModifiers() == 16) { // left mousebutton dragged
			...
			// (oldX-me.getX())*zoomFactor);
			// System.out.println("heightoffset: " + heightoffset + " " +
			// (oldY-me.getY())*zoomFactor);
			// computeNewCoors();
			widthoffset = tempWidthOffset;
			heightoffset = tempHeightOffset;
			tempWidthOffset = 0;
		...
		if (me.getModifiers() == 4) { // right mousebutton dragged
			// mouseDragged = true;
			/*
			 * if ((oldY-me.getY()) > 0){ this.zoffset = -1; }else{ this.zoffset = 1; }
			 */
	...
	private Coors searchCoors(int find_x, int find_y) {
		...
		while ((findCoor == null) & (i < pcoors.size())) {
			...
			if ((!aCoors.isOrigo()) & (!aCoors.isConnection())) {
				...
				if ((x_diff < 8) & (y_diff < 8)) { // om spelaren klickat inom 8 pixlar fr�n en planet
					findCoor = aCoors;
				}
			}
			i++;
	...
	private void reset() {
		...
		setConnections(connections, SpaceRazePanel.galaxy);
		computeNewOrigo();
		zvalue = 100;
	}

	public void doSetCenter(String uuid) {
		newCenterString = uuid;
	}
	...
	private void setCenter(String uuid) {
		Logger.finer("setCenter called, uuid=" + uuid);
		double newcenterx = 0.0;
		double newcentery = 0.0;
		double newcenterz = 0.0;
		Coors newcentercoor = findCoors(uuid);
	...
	private Coors findCoors(String uuid) {
		Coors found = null;
		int i = 0;
		while ((i < pcoors.size()) & (found == null)) {
			Coors thiscoor = pcoors.get(i);
			if (thiscoor.getMapPlanetUuid().equalsIgnoreCase(uuid)) {
				found = thiscoor;
			} else if (uuid.equalsIgnoreCase("Sector origo") && thiscoor.isOrigo()) {
				found = thiscoor;
			} else {
				i++;
			}
	...
	public void paint(Graphics g) {
		if (once) {
			...
			if (initialSetCenter != null) {
				...
				setCenterName = player.getHomePlanet().getMapPlanetUuid();
			...
			if (buffer != null) { // denna komponent �r ej synlig, avbryt paint och avsluta tr�den
				...
				for (int i = 0; i < pcoors.size(); i++) {
					...
					// bg.setColor(new
					// Color(((15*size*cc.getRed())/255),((15*size*cc.getGreen())/255),((15*size*cc.getBlue())/255)));
					bg.setColor(cc); // ej fade:a f�rgen
					if (!c.isOrigo()) {
						if (!c.isConnection()) {
							Planet tempPlanet = PlanetPureFunctions.getPlanet(c.getMapPlanetUuid(), SpaceRazePanel.galaxy);
							String planetOwner = null;
							if (tempPlanet.getPlayerInControl() != null) {
								planetOwner = tempPlanet.getPlayerInControl().getGovernorName();
							}
							boolean spy = VipPureFunctions.findVIPSpy(tempPlanet, player, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld) != null;
							boolean shipInSystem = PlayerPureFunctions.playerHasShipsInSystem(player, tempPlanet, SpaceRazePanel.galaxy);
							boolean troopInSystem = false;
							if (TroopPureFunctions.getTroopsOnPlanet(tempPlanet, player, SpaceRazePanel.galaxy.getTroops(), SpaceRazePanel.gameWorld).size() > 0) {
								troopInSystem = true;
							}
							...
							if (chosenCoors.equalsIgnoreCase(c.getMapPlanetUuid())) {
								drawChosenPlanet(bg, newx, newy);
							}
							// bg.setColor(new Color(cc.getRed()/2,cc.getGreen()/2,cc.getBlue()/2)); //
							// faded ring to make softer edge on planets
							...
							boolean lastKnownRazed = PlanetPureFunctions.findPlanetInfo(tempPlanet.getMapPlanetUuid(), player.getPlanetInformations()).isRazed();
							boolean razed = PlanetPureFunctions.isRazed(tempPlanet) & (tempPlanet.getPlayerInControl() == null);
							if ((razed & (shipInSystem | spy | troopInSystem)) | lastKnownRazed) {
								bg.setColor(new Color(255, 63, 63));
								bg.drawLine(newx - (size / 2) - 2, newy - (size / 2) - 2, newx + (size / 2) + 2,
										newy + (size / 2) + 2);
								bg.drawLine(newx - (size / 2) - 2, newy + (size / 2) + 2, newx + (size / 2) + 2,
										newy - (size / 2) - 2);
							} else if (c.isBesieged()) {
								bg.setColor(new Color(255, 191, 63));
								bg.drawOval(newx - (size / 2) - 2, newy - (size / 2) - 2, size + 5, size + 5);
							}
							// bg.setColor(new
							// Color(((15*size*cc.getRed())/255),((15*size*cc.getGreen())/255),((15*size*cc.getBlue())/255)));
							bg.setColor(new Color(41, 198, 255)); // ljusbl� text
							bg.setFont(new Font("Helvetica", 0, 12));
							bg.drawString(c.getMapPlanetUuid(), (newx + (size / 2) + 2), (newy));
							// create the buildings string
							String buildings = c.getBuildingsString(spy, shipInSystem, troopInSystem, tempPlanet,
									player);
							// draw production and resistance
							if (c.showValues()) {
								String troopsString = "";
								Planet p = PlanetPureFunctions.getPlanet(c.getMapPlanetUuid(), SpaceRazePanel.galaxy);
								...
								if (c.isRazed()) {
									if (c.isRazedAndUninfected()) {
										...
										// alien player
										bg.drawString("- / " + String.valueOf(c.getResistance()) + troopsString,
												(newx + (size / 2) + 2) + 6, (newy - 12));
									}
								} else {
									bg.drawString(
											String.valueOf(c.getProduction()) + " / "
													+ String.valueOf(c.getResistance()) + troopsString,
											(newx + (size / 2) + 2) + 6, (newy - 12));
								}
								// bg.setFont(new Font("Helvetica",0,12));
								// bg.setColor(new Color(41,198,255)); // ljusbl� text

							} else if (c.showLastKnownValues()) {
								...
								// bg.drawString(tmpProd + " / " +
								// String.valueOf(c.getLastKnownRes()),(newx+(size/2)+2)+6,(newy-12));
								bg.drawString(tmpProd + " / " + String.valueOf(c.getLastKnownRes()) + tmpTroops,
										(newx + (size / 2) + 2) + 6, (newy - 12));
								// bg.setFont(new Font("Helvetica",0,12));
								// bg.setColor(new Color(41,198,255)); // ljusbl� text
							}
							...
							// Logger.finest("Planet: " + tempPlanet.getName() + " buildings: " +
							// buildings);
							if (!buildings.equals("")) {
								bg.setColor(cc);
								bg.setFont(new Font("Helvetica", 0, 10));
								...
								if ((c.getLastKnownBuildingsString() != null)
										&& !c.getLastKnownBuildingsString().equals("")) {
									Logger.finest(
											"c.getLastKnownBuildingsString(): " + c.getLastKnownBuildingsString());
									...
									bg.drawString(c.getLastKnownBuildingsString() + ", ",
											newx - (size / 2) - 3 - wWidth - wWidth2, (newy - 12));
								}
								...
								if ((c.getLastKnownBuildingsString() != null)
										&& !c.getLastKnownBuildingsString().equals("")) {
									Logger.finest(
											"c.getLastKnownBuildingsString() 2: " + c.getLastKnownBuildingsString());
									bg.setColor(Color.GRAY);
									bg.setFont(new Font("Helvetica", 0, 10));
									FontMetrics fm = bg.getFontMetrics();
									int wWidth = fm.stringWidth(c.getLastKnownBuildingsString());
									bg.drawString(c.getLastKnownBuildingsString(), newx - (size / 2) - 3 - wWidth,
											(newy - 12));
								}
							}
							...
							bg.setFont(new Font("Helvetica", 0, 12));
							FontMetrics fm2 = bg.getFontMetrics();
							int strWidth2 = fm2.stringWidth(c.getMapPlanetUuid());
							String sizeStr = "";
							...
							if (c.isOpen()) {
								...
								bg.drawLine((newx + (size / 2) + 2 + strWidth + 4), (newy - 1),
										(newx + (size / 2) + 2 + strWidth + 4), (newy - 9));
								bg.drawLine((newx + (size / 2) + 2 + strWidth + 1), (newy - 5),
										(newx + (size / 2) + 2 + strWidth + 7), (newy - 5));
								bg.fillRect((newx + (size / 2) + 2 + strWidth + 3), (newy) - 6, 3, 3);
							} else {
								tmpX += fm3.stringWidth(sizeStr);
							}
							if (showPlanetNotes) {
								PlanetNotesChange aPlanetNotesChange = OrderPureFunctions.getPlanetNotesChange(player.getOrders(), tempPlanet);
								String planetNotes = null;
								if (aPlanetNotesChange != null) {
									planetNotes = aPlanetNotesChange.getNotesText();
								} else {
									planetNotes = PlanetPureFunctions.findPlanetInfo(tempPlanet.getMapPlanetUuid(), player.getPlanetInformations()).getNotes();
								}
							...
							// sort the ships
							Collections.sort(tempshortsShips, new SpaceshipSizeAndBuildCostComparator(SpaceRazePanel.gameWorld));
							int currentx = 0;
							int shipCount = 0;
							...
							for (int l = 0; l < tempshortsShips.size(); l++) {
								...
								bg.setColor(ColorConverter
										.getColorFromHexString(GameWorldHandler.getFactionByUuid(tempss.getOwner().getFactionUuid(), SpaceRazePanel.gameWorld).getPlanetHexColor()));
								boolean drawNow = false;
								if (l == (tempshortsShips.size() - 1)) {
									drawNow = true;
								} else {
									Spaceship tempss2 = tempshortsShips.get(l + 1);
									if (!SpaceshipPureFunctions.getSpaceshipTypeByUuid(tempss2.getTypeUuid(), SpaceRazePanel.gameWorld).getName()
											.equals(SpaceshipPureFunctions.getSpaceshipTypeByUuid(tempss.getTypeUuid(), SpaceRazePanel.gameWorld).getName())) {
										drawNow = true;
									}
								}
								if (drawNow) {
									String tmpShipStr = SpaceshipPureFunctions.getSpaceshipTypeByUuid(tempss.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName();
									if (shipCount > 1) {
									...
									if (tmpVips.size() > 0) {
										...
										for (Iterator<VIP> iter = tmpVips.iterator(); iter.hasNext();) {
											VIP aVIP = iter.next();
											tmpShipStr = tmpShipStr + VipPureFunctions.getVipTypeByUuid(aVIP.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName();
											if (iter.hasNext()) {
												tmpShipStr = tmpShipStr + ",";
											}
										}
							...
							Collections.sort(tempTroops, new TroopTypeAndBuildCostComparator(SpaceRazePanel.gameWorld));
							...
							for (Troop troop : tempTroops) {
								String name = TroopPureFunctions.getTroopTypeByUuid(troop.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName();
								...
								troopsMap.put(TroopPureFunctions.getTroopTypeByUuid(troop.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName(), tempCounter);
							}
							...
							if (tempTroops.size() > 0) {
								...
								for (Iterator<String> iter = troopsMap.keySet().iterator(); iter.hasNext();) {
									...
									for (Troop aTroop : tempTroops) {
										...
										if (TroopPureFunctions.getTroopTypeByUuid(aTroop.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName().equals(aTroopTypeName)) {
									...
									if (vipsOnTroops.size() > 0) {
										...
										for (Iterator<VIP> iter2 = vipsOnTroops.iterator(); iter2.hasNext();) {
											VIP aVIP = iter2.next();
											tmpTroopStr = tmpTroopStr + VipPureFunctions.getVipTypeByUuid(aVIP.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName();
											if (iter2.hasNext()) {
												tmpTroopStr = tmpTroopStr + ",";
											}
										}
							...
							for (int l = 0; l < tempshortsVIPs.size(); l++) {
								...
								// System.out.println("VIP location: " + tempv.getLocation().getName() + " " +
								// c.getName());
								...
								if ((tempv.getShipLocation() == null) & (tempv.getTroopLocation() == null)) {
									...
									if ((tempv.getBoss() == player) | (c.isOpen())) {
										bg.setColor(ColorConverter.getColorFromHexString(
												GameWorldHandler.getFactionByUuid(tempv.getBoss().getFactionUuid(), SpaceRazePanel.gameWorld).getPlanetHexColor()));
										String tmpName = VipPureFunctions.getVipTypeByUuid(tempv.getTypeUuid(), SpaceRazePanel.gameWorld).getShortName();
						...
						// bg.drawOval(newx-((int)Math.round((min*(100.0/zvalue)))/2),newy -
						// ((int)Math.round((min*(100.0/zvalue)))/2),(int)Math.round(min*(100.0/zvalue)),(int)Math.round(min*(100.0/zvalue)));
						bg.drawOval(newx - 2, newy - 2, 4, 4);
					}
				}
				// skriv ut zoom
				bg.setColor(StyleGuide.colorCurrent.darker()); // gr�n text
				bg.setFont(new Font("Helvetica", 0, 12));
				bg.drawString("Map: " + SpaceRazePanel.galaxy.mapVersion(), 5, 13);
				 ...
				 * map control buttons.
				 * 
				 * bg.setColor(StyleGuide.colorCurrent); bg.setFont(new Font("Dialog",1,12));
				 * bg.drawRect(30, 80, 40, 20); bg.drawString("Up",45,94);
				 * 
				 * bg.drawRect(10, 100, 40, 20); bg.drawString("Left",20,114);
				 * 
				 * bg.drawRect(50, 100, 40, 20); bg.drawString("Right",57,114);
				 * 
				 * bg.drawRect(30, 120, 40, 20); bg.drawString("Down",35,134);
				 * 
				 */

				// bg.drawString("R offset: " +
				// Functions.formatString(String.valueOf(roffset),2),5,59);
				// rita ut bufferten
	...
	private void drawSelectedPlanet(Graphics g, int x, int y, Color aColor) {
		...
		for (int i = 0; i < nrLines; i++) { // iterate over the lines
			...
			int relX = (int) Math.round(lineLength * Math.sin(angleRad));
			int relY = (int) Math.round(lineLength * Math.cos(angleRad));
			int colorValueRed = (int) Math.abs(Math.round(aColor.getRed() / 2 * Math.sin(animation2 + angleRad)))
					+ aColor.getRed() / 2;
			int colorValueGreen = (int) Math.abs(Math.round(aColor.getGreen() / 2 * Math.sin(animation2 + angleRad)))
					+ aColor.getGreen() / 2;
			int colorValueBlue = (int) Math.abs(Math.round(aColor.getBlue() / 2 * Math.sin(animation2 + angleRad)))
					+ aColor.getBlue() / 2;
	...
	private void drawOtherFleets(Coors aCoors, int x, int y, Graphics g) {
		// h�mta planeten
		Planet tempPlanet = PlanetPureFunctions.getPlanet(aCoors.getMapPlanetUuid(), SpaceRazePanel.galaxy);
		String shipSize = "";
		...
		// int counter = 0;
		if (SpaceRazePanel.galaxy.isPlayerShipAtPlanet(player, tempPlanet) || troopInSystem
				|| tempPlanet.getPlayerInControl() == player || tempPlanet.isOpen()
				|| VipPureFunctions.findVIPSpy(tempPlanet, player, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld) != null) {
			List<Player> allp = SpaceRazePanel.galaxy.getPlayers();
			// loopa igenom alla spelare och kolla efter flottor
			for (int i = 0; i < allp.size(); i++) {
				Player tempPlayer = (Player) allp.get(i);
				if (tempPlayer != player) {
					shipSize = GalaxyMapPureFunctions.getLargestShipSizeOnPlanet(tempPlanet, tempPlayer, false, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld);
					boolean civilianExists = !GalaxyMapPureFunctions
							.getLargestShipSizeOnPlanet(tempPlanet, tempPlayer, true, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld).equals("");
					if (civilianExists) {
						if (shipSize.equals("")) {
							shipSize = "civ";
						} else {
					...
					if (!shipSize.equalsIgnoreCase("")) {
						g.setColor(ColorConverter.getColorFromHexString(GameWorldHandler.getFactionByUuid(tempPlayer.getFactionUuid(), SpaceRazePanel.gameWorld).getPlanetHexColor()));
			...
			shipSize = GalaxyMapPureFunctions.getLargestShipSizeOnPlanet(tempPlanet, null, false, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld);
			if (!shipSize.equalsIgnoreCase("")) {
	...
	private void drawOtherTroops(Coors aCoors, int x, int y, Graphics g) {
		...
		boolean surveyShip = SpaceshipPureFunctions.findSurveyShip(tempPlanet, player, SpaceRazePanel.galaxy.getSpaceships(), SpaceRazePanel.gameWorld) != null;
		boolean surveyVIP = VipPureFunctions.findSurveyVIPonShip(tempPlanet, player, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld) != null;

		if (tempPlanet.getPlayerInControl() == player || VipPureFunctions.findVIPSpy(tempPlanet, player, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld) != null
			...
			for (int i = 0; i < allp.size(); i++) {
				...
				if (tempPlayer != player && tempPlayer != tempPlanet.getPlayerInControl()) {
					int numberOfTroops = TroopPureFunctions.getTroopsOnPlanet(tempPlanet, tempPlayer, SpaceRazePanel.galaxy.getTroops(), SpaceRazePanel.gameWorld).size();
					if (numberOfTroops > 0) {
	...
	private void computeNewCoors() {
		...
		for (int i = 0; i < pcoors.size(); i++) {
			...
			if (c.isConnection()) { // r�kna ut det andra upps�ttningen koordinater f�r linjer
	...
	private void insertCoorSorted(Coors c, List<Coors> inscoors) {
		...
		if (c.isConnection()) {
			...
			while ((!found) & (i < inscoors.size())) {
				...
				if (!temp.isConnection() & !temp.isOrigo()) { // om ej connection eller origo kolla z-v�rdet
					if (c.getZ() < temp.getZ()) {
						inscoors.add(i, c);
						found = true;
					} else {
				...
				} else { // connections och origo skall alltid ligga f�rst
					i++;
				}
			}
			if (!found) {
	...
	public void setPlanets(List<Planet> planets, Player aPlayer) {
		...
		for (int i = 0; i < planets.size(); i++) {
			...
			boolean spy = VipPureFunctions.findVIPSpy(planet, aPlayer, SpaceRazePanel.galaxy, SpaceRazePanel.gameWorld) != null;

			boolean troopInSystem = false;
			if (TroopPureFunctions.getTroopsOnPlanet(planet, aPlayer, SpaceRazePanel.galaxy.getTroops(), SpaceRazePanel.gameWorld).size() > 0) {
				troopInSystem = true;
			}
			boolean surveyShip = SpaceshipPureFunctions.findSurveyShip(planet, aPlayer, SpaceRazePanel.galaxy.getSpaceships(), SpaceRazePanel.gameWorld) != null;
			...
			// c = new Coors(planet.getXcoor()-50, planet.getYcoor()-50, planet.getZcoor()-50); varf�r -50?
            MapPlanet mapPlanet = GalaxyMapPureFunctions.getPlanet(planet.getMapPlanetUuid(), SpaceRazePanel.galaxyMap);
            c = new Coors(mapPlanet.getX(), mapPlanet.getY(), mapPlanet.getZ());
			c.setMapPlanetUuid(mapPlanet.getName());
			c.setOpen(planet.isOpen());
			// set prod / res
			if ((planet.isOpen()) | (planet.getPlayerInControl() == aPlayer) | (spy) | (surveyShip | surveyVIP | troopInSystem)) {
				c.setProduction(planet.getPopulation());
				c.setResistance(planet.getResistance());
				int nrTroops = getNrTroops(planet, SpaceRazePanel.galaxy);
				c.setNrTroops(nrTroops);
			} else {
				// set last known prod & res
				int tmpProd = PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getProd();
				int tmpRes = PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getRes();
				c.setLastKnownProd(tmpProd);
				c.setLastKnownRes(tmpRes);
				c.setLastKnownTroopsNr(PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getLastKnownTroopsNr());
			}
			// maybe set last known max ship size
			if ((planet.getPlayerInControl() != aPlayer) & !PlayerPureFunctions.playerHasShipsInSystem(aPlayer, planet, SpaceRazePanel.galaxy)
					& !planet.isOpen() & !spy & !troopInSystem) {
				c.setLastKnownMaxShipSize(PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getLastKnownMaxShipSize());
			}
			// set building info
			if ((planet.isOpen()) | (planet.getPlayerInControl() == aPlayer) | (spy) | troopInSystem) {
				if (planet.getBuildings().size() > 0) {
					c.setBuilding(planet.getBuildings());
				}
			} else if (PlayerPureFunctions.playerHasShipsInSystem(aPlayer, planet, SpaceRazePanel.galaxy)) {
				c.setBuilding(PlanetPureFunctions.getBuildings(planet, true, SpaceRazePanel.gameWorld));
				// handle that one maybe have info on old planetbased buildings
				c.setLastKnownBuildingsString(PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getLastKnownBuildingsOnSurface());

			} else {
				c.setLastKnownBuildingsString(PlanetPureFunctions.getLastKnownBuildings(planet.getMapPlanetUuid(), player.getPlanetInformations()));
			}
			if (planet.isBesieged() & ((aPlayer == planet.getPlayerInControl())
					| (PlayerPureFunctions.playerHasShipsInSystem(aPlayer, planet, SpaceRazePanel.galaxy) | spy | troopInSystem))) {
				c.setBesieged();
			}
			...
			if ((tempPlayer == aPlayer) | (planet.isOpen()) | (PlayerPureFunctions.playerHasShipsInSystem(aPlayer, planet, SpaceRazePanel.galaxy)) | spy
					| troopInSystem) { // spelarens egen planet, visa r�tt f�rg
				if (planet.getPlayerInControl() != null) {
					c.setColor(ColorConverter.getColorFromHexString(GameWorldHandler.getFactionByUuid(tempPlayer.getFactionUuid(), SpaceRazePanel.gameWorld).getPlanetHexColor()));
				} else {
					c.setColor(StyleGuide.colorNeutralWhite);
				}
			} else { // annans eller neutral planet, visa senaste k�nda info/f�rg
				if (PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getLastKnownOwner().equalsIgnoreCase("Neutral")) { // neutral planet
					c.setColor(StyleGuide.colorNeutralWhite);
				} else {
					c.setColor(ColorConverter.getColorFromHexString(GameWorldHandler.getFactionByUuid(SpaceRazePanel.galaxy
							.getPlayerByUserName(PlanetPureFunctions.findPlanetInfo(planet.getMapPlanetUuid(), aPlayer.getPlanetInformations()).getLastKnownOwner()).getFactionUuid(), SpaceRazePanel.gameWorld).getPlanetHexColor()));
				}
			}
			insertCoorSorted(c, pcoors);
		}
	...
	private int getNrTroops(Planet aPlanet, Galaxy galaxy) {
				...
				if (aTroop.getPlanetLocation() != null) {
					if (aTroop.getPlanetLocation() == aPlanet) {
						if (aPlanet.getPlayerInControl() == aTroop.getOwner()) {
							if (TroopPureFunctions.getTroopTypeByUuid(aTroop.getTypeUuid(), SpaceRazePanel.gameWorld).isVisible()) {
								nrTroopsOnPlanet++;
							}
						}
					}
	...
	public void setSpaceships(List<Spaceship> spaceships) {
		...
		for (int i = 0; i < spaceships.size(); i++) {
			tempss = spaceships.get(i);
			if (tempss.getLocation() != null) { // skepp som flyr syns ej p� kartan
				foundCoor = findCoors(tempss.getLocation().getMapPlanetUuid());
				if (foundCoor != null) {
	...
	public void setOwnVips(List<VIP> vips) {
		...
		for (int i = 0; i < vips.size(); i++) {
			...
			// LoggingHandler.fine("VIP location == null: " + (tempv.getLocation() ==
			// null));
			// LoggingHandler.fine("VIP: " + tempv.getName() + " " +
			// tempv.getLocationString());
			if (VipPureFunctions.getLocation(tempv) != null) {
				foundCoor = findCoors(VipPureFunctions.getLocation(tempv).getMapPlanetUuid());
				if (foundCoor != null) {
	...
	public void setOwnTroops(List<Troop> ownTroops) {
		...
		for (Troop aTroop : troops) {
			if (aTroop.getPlanetLocation() != null) {
				foundCoor = findCoors(aTroop.getPlanetLocation().getMapPlanetUuid());
				if (foundCoor != null) {
					foundCoor.addTroop(aTroop);
				}
			} else if (aTroop.getShipLocation() != null) {
				if (aTroop.getShipLocation().getLocation() != null) {
					foundCoor = findCoors(aTroop.getShipLocation().getLocation().getMapPlanetUuid());
					if (foundCoor != null) {
						foundCoor.addTroop(aTroop);
					}
				}
	...
	public void setOthersVips(List<VIP> allVips) {
		...
		for (int i = 0; i < allVips.size(); i++) {
			...
			if (VipPureFunctions.getLocation(tempv) != null) {
				foundCoor = findCoors(VipPureFunctions.getLocation(tempv).getMapPlanetUuid());
				if (foundCoor != null) {
					if (!tempv.getBoss().getName().equalsIgnoreCase(player.getName())) {
						// LoggingHandler.fine("other player found!");
						if (VipPureFunctions.getVipTypeByUuid(tempv.getTypeUuid(), SpaceRazePanel.gameWorld).getShowOnOpenPlanet()) {
	...
	public void setConnections(List<PlanetConnection> connections, Galaxy galaxy) {
		...
		for (int i = 0; i < connections.size(); i++) {
			...
			Planet p1 = galaxy.getPlanet(pc.getPlanetOne().getMapPlanetUuid()); //TODO vi har Planet från pc.getPlanetOne(), varför tar vi omvägen? ska vi bara spra guuid i pc.getPlanetOne()?
			Planet p2 =galaxy.getPlanet(pc.getPlanetTwo().getMapPlanetUuid());
            MapPlanet mapPlanet1 = GalaxyMapPureFunctions.getPlanet(pc.getPlanetOne().getMapPlanetUuid(), SpaceRazePanel.galaxyMap);
            MapPlanet mapPlanet2 = GalaxyMapPureFunctions.getPlanet(pc.getPlanetTwo().getMapPlanetUuid(), SpaceRazePanel.galaxyMap);
            c = new Coors(mapPlanet1.getX(), mapPlanet1.getY(), mapPlanet1.getZ(), mapPlanet2.getX(), mapPlanet2.getY(), mapPlanet2.getZ(),
					pc.isLongRange());
			c.setMapPlanetUuid("");
			if (!pc.isLongRange()) {
				...
				if ((p1.getPlayerInControl() != null) & (p2.getPlayerInControl() != null)) { // none of the planets are
					...
					if (PlanetPureFunctions.hasSpacePort(p1) && PlanetPureFunctions.hasSpacePort(p2)) { // both have a spacestation
						if (DiplomacyPureFunctions.friendlySpaceports(p1.getPlayerInControl(),
								p2.getPlayerInControl(), SpaceRazePanel.galaxy.getDiplomacyStates())) {
							if (DiplomacyPureFunctions.friendlySpaceports(player, p1.getPlayerInControl(), SpaceRazePanel.galaxy.getDiplomacyStates())) {
